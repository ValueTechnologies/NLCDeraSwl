<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageD.master" AutoEventWireup="true" CodeBehind="ViewPurchaseOrders.aspx.cs" Inherits="NLCDeraSwl.ViewPurchaseOrders" %>
    ...
    $(document).ready(function () {
        ...
        function LoadPOFor() {
            $.ajax({
                ...
                dataType: "json",
                url: "POSSupplierRegistration.aspx/GetSupplierForList",
                data: "{ }",
                success: function (response) {
                    try {
                        var jData = $.parseJSON(response.d), Out = "", OutDDL = "";
                        Out = Out + '<option value="" selected>----- ALL -----</option>';
                        $.each(jData, function (i, item) {
                            Out = Out + '<option value="' + item.ID + '"> ' + item.SupplierFor + '</option>';
                        });
                        $('#ddlPOFor').html(Out);
                    } catch (e) {
                        alert(e.message);
        ...
        function GetPurchaseOrders() {
            $.ajax({
                ...
                data: "{ 'POFor' : '" + $('#ddlPOFor').val() + "', 'fromDate' : '" + $('#txtFrom').val() + "', 'toDate' : '" + $('#txtTo').val() + "' }",
                success: function (response) {
                    try {
                        debugger;
                        ...
                        $.each(jData, function (i, item) {
                            ...
                            if (pid != item.PurchaseOrderID && first) {
                                ...
                                Out = Out + '<a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse' + item.PurchaseOrderID + '" aria-expanded="true" aria-controls="collapse' + item.PurchaseOrderID + '"> ' + item.PONumber + ' (' + date + ') (' + item.ShopAbbv + ') </a> </h4> </div>';
                    ...
                    <form class="form-horizontal" action="#">
                        ...
                        <div class="form-row row-fluid">
                            <div class="span12">
                                <div class="form-row row-fluid">
                                    <div class="span 6">
                                        <label class="form-label span2" for="normal">PO For : </label>
                                        <div class="controls sel">
                                            <select class="nostyle frmCtrl span4" id="ddlPOFor">
                                            </select>
                                        </div>
                                    </div>
                                </div>
