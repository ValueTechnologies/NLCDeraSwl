<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageD.master" AutoEventWireup="true" CodeBehind="POSReceipt.aspx.cs" Inherits="NLCDeraSwl.POSReceipt" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            function SearchProduct() {
                //alert("Shop ID : " + "<%= HttpContext.Current.Session["ShopID"]%>");
                $.ajax({
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    ...
                    success: function (response) {
                        try {
                            ...
                            $.each(jData, function (i, item) {
                                Out = Out + '<tr><td>' + item.Srno + '</td><td style="text-align : left;">' + item.ProductName + ' </td> <td style="text-align : left;">' + item.Features + ' </td> <td style="text-align : right;">' + item.Qty + ' </td> <td style="text-align : right;">' + item.Price + '</td>  <td><button price="' + item.Price + '" tag="' + item.ProductID + '" type="button" class="btn btn-link clsProduct">Select</button>   </td> </tr>';
                            });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('.clsQty').each(function (i, item) {
                            UnitPrice[i] = parseInt($(this).parent().next().html(), 10);
                        });
                        ...
                        for (var index = 0; index < ProductId.length ; index++) {
                            $.ajax({
                                ...
                                data: "{    'ReciptID' : '" + SalesID + "', 'ProductID' : '" + ProductId[index] + "',   'Qty' : '" + Qty[index] + "', 'UnitPrice' : '" + UnitPrice[index] + "' }",
                                success: function (response) {
            ...
            function CalculateItemPrice() {
                try {
                    $('.clsQty').each(function (i, item) {
                        ...
                        $(this).parent().next().next().html(qty * price);
                    });
